...
✅ 코드잇 스프린트 주강사님 제안 하이브리드 구조 (Outer Graph + Inner Toggle)
...
└────────────────────────────┬─────────────────────────────────────┘
  ...
  ╔══════════════╗   ┌──────────────┐    ┌──────────────────┐
  ║ 라우트 A      ║   │ 라우트 B      │    │ 라우트 C          │
  ║ 단순 RAG      ║   │ Structure    │    │ Compliance       │
  ║              ║   │ Mapper 도구   │    │ Validator 도구    │
  ║              ║   │              │    │                  │
  ║ ⭐ [2단      ║   │ • 메타필터링   │    │ • 필수/반드시     │
  ║   라우팅]     ║   │   검색 (도구   │    │   키워드 검색     │
  ║              ║   │   특화)       │    │   (도구 특화)     │
  ║ config       ║   │ • LLM 구조화  │    │ • LLM 체크리스트   │
  ║ .retriever_  ║   │   추출        │    │   생성            │
  ║ type 분기     ║   └──────┬───────┘    └──────┬───────────┘
  ║              ║          │                   │
  ║ ┌──────────┐ ║          │                   │
